...
import os
from typing import Collection, Mapping, Sequence, List, Dict, Tuple
...
# -------------------------------------------------------------
# RNG utilities (per-process RNG for multiprocessing safety)
# -------------------------------------------------------------
_RND = None
...
def mutate_alleles(marker: Marker, source_alleles: List[Allele], two_step_mutation_factor: float, biases: List[Bias]) -> Tuple[List[Allele], float, float]:
    rnd = _rng()
    ...
    return sorted(mutated_alleles, key=lambda a: a.value), weighted_prob_prod, unweighted_prob_prod


def mutate_haplotype(source: Haplotype, marker_set: MarkerSet, two_step_mutation_factor: float, biases: List[Bias]) -> Tuple[Haplotype, float, float]:
    target = Haplotype()
    w_edge = 1.0
    u_edge = 1.0
...
def get_edge_probability(source: Haplotype, target: Haplotype, marker_set: MarkerSet, two_step_mutation_factor: float, is_average_pedigree: bool) -> Decimal:
    edge = Decimal(1)
    for marker in marker_set.markers:
...
def get_edge_probabilities(haplotypes: Mapping[str, Haplotype], relationships: Collection[Relationship], marker_set: MarkerSet, two_step_mutation_factor: float, is_average_pedigree: bool) -> Dict[Tuple[str, str], Decimal]:
    return {
...
# -------------------------------------------------------------
# Inside: Average pedigree probability
# -------------------------------------------------------------
...
def simulate_pedigree_probability(
    ...
    ordered_unknown = [ind.id for ind in pedigree.get_level_order_traversal(root_name) if ind.haplotype_class == "unknown"]
    ...
    unused = [rel for rel in pedigree.relationships if (rel.parent_id, rel.child_id) not in simulated and (rel.child_id, rel.parent_id) not in simulated]
...
def simulate_pedigree_iteration(i: int, root_name: str, pedigree: Pedigree, marker_set: MarkerSet, two_step_mutation_factor: float, bias_value: float = 0.1) -> IterationResult:
    return simulate_pedigree_probability(pedigree, root_name, marker_set, two_step_mutation_factor, bias_value)

# -------------------------------------------------------------
# Pool machinery for batches
# -------------------------------------------------------------
...
def _a1_run_batch(args):
    ...
    for j in range(start, start + count):
        ...
        if is_inside and prob != 0:
            ...
            try:
                matches = [uid for uid in ordered_unknown_ids if (haplotypes.get(uid) == suspect) and (not individuals[uid].exclude)]
            except KeyError:
                matches = []
...
# -------------------------------------------------------------
# Common processing loop (used by inside/outside and average pedigree)
# -------------------------------------------------------------
...
def process_iteration_results(
    ...
    with progress_bar:
        ...
        final_per_individual: Dict[str, Decimal] = {}
        ...
        while not valid:
            if dynamic_bias_enabled:
                ...
                try:
                    ...
                    reporter.log("\nDynamic bias schedule (cycle/trial {}): {}".format(trial, schedule))
                except Exception:
                    pass

            for m in range(3):
                out_path = f"{simulation_parameters.results_path}/{out_file_name}_m_{m}_outside_{is_outside}.txt"
                with open(out_path, "a", buffering=1) as f:
                    if dynamic_bias_enabled:
                        ...
                        current_partial = partial(base_func, *base_args, **base_kwargs)
                    ...
                    with multiprocessing.Pool(
                        threads,
                        initializer=_a1_pool_init,
                        initargs=(current_partial.func, getattr(current_partial, 'keywords', {}) or {})
                    ) as pool:
                        print("\nStarting trial {} , model {} with {} threads (batch_size={})...".format(trial, m + 1, threads, batch_size))
                        for batch_out, per_ind_dict, per_pair_dict in pool.imap_unordered(_a1_run_batch, tasks, chunksize=1):
                            n_new = len(batch_out)
                            for prob, w in batch_out:
                                model_iterations[m] += 1
                                weight_sums[m] += Decimal(w)
                                weighted_sums[m] += (prob * Decimal(w))
                                current_mean = (weighted_sums[m] / weight_sums[m]) if weight_sums[m] != 0 else Decimal(0)
            ...
            number_outside = sum(sum(abs(prob - total_mean) / total_mean > simulation_parameters.model_validity_threshold for prob in model_probabilities[m]) for m in range(3))

            all_inside = all(
                all(abs(prob - total_mean) / total_mean < simulation_parameters.model_validity_threshold for prob in model_probabilities[m])
                for m in range(3)
            )
            if all_inside:
                if total_mean > 1.0:
                    reporter.log(
                        "\nModel is stable after {} trials. However, current mean: {} (log {}) is greater than unity. Starting new trial with tightened threshold: {}".format(
                            trial, total_mean, _safe_log10dB(total_mean), (simulation_parameters.model_validity_threshold / 2)
                        )
                    )
                ...
                reporter.log(
                    ("\nModel is not valid after {} trials. Current mean: {} (log {}). {} data points fall outside range. Starting new trial...").format(
                        trial, total_mean, _safe_log10dB(total_mean), number_outside
                    )
                )
            if not valid:
                trial += 1
        ...
        try:
            if out_file_name == "match_probabilities" and (not is_outside):
                ...
                ctx_inds = base_kwargs.get("individuals", {})
                for uid in all_ids:
                    vals = [d.get(uid, Decimal(0)) for d in per_model_means]
                    # Use individual name instead of ID
                    ind = ctx_inds.get(uid) if isinstance(ctx_inds, dict) else None
                    if ind is not None:
                ...
                csv_path = f"{simulation_parameters.results_path}/per_individual_marginal_probabilities_{ts}.csv"
                with open(csv_path, "w", newline="") as csvfile:
                    writer = csv.writer(csvfile)
                    writer.writerow(["individual_id", "individual_name", "probability"])
                    ordered = base_kwargs.get("ordered_unknown_ids", [])
                    for uid in ordered:
                        ind = ctx_inds.get(uid) if isinstance(ctx_inds, dict) else None
                        if ind is None or getattr(ind, "exclude", False):
                            continue
                        name = getattr(ind, "name", str(uid))
                        writer.writerow([uid, name, str(final_per_individual.get(name, Decimal(0)))])
                reporter.log(f"Saved per-individual marginal probabilities to: {csv_path}")
        except Exception as _csv_err:
            reporter.log(f"[warn] Could not write per-individual/pairwise CSVs: {_csv_err}")
...
def simulate_matching_haplotypes(pedigree: Pedigree, individuals: Mapping[str, Individual],
                                 ...
                                 is_outside: bool, bias_value: float = 0.1) -> IterationResult:
    rnd = _rng()
    ...
    haplotypes = {ind.id: (deepcopy(suspect_haplotype) if ind.id == fixed_individual_id else individuals[ind.id].haplotype)
    ...
    try:
        ...
        if missing:
            print(f"[warn] fixed individual {fixed_individual_id} missing loci for bias: {missing}")
    except Exception:
        pass
    ...
    for uid in ordered_unknown_ids:
        ...
        try:
            ...
            if 'empty locus' in str(_e).lower():
                biases = []
                print(f"[warn] get_biases empty locus for id={uid}; continuing without bias")
            else:
                raise
        haplotypes[uid], w, u = mutate_haplotype(parent, marker_set, two_step_mutation_factor, biases)
        w_total *= w
    ...
    non_excl_match = [uid for uid in ordered_unknown_ids if (haplotypes[uid] == suspect_haplotype) and (not individuals[uid].exclude)]
    ...
    if not is_outside:
        probability = (cond / (simulation_probability * total_number)) if (number_non_excl >= 1 and simulation_probability != 0) else Decimal(0)
    else:
...
def calculate_matching_haplotypes(pedigree: Pedigree, marker_set: MarkerSet, root_name: str,
    ...
    ordered_unknown_ids = [ind.id for ind in pedigree.get_level_order_traversal(root_name) if ind.haplotype_class == "unknown"]
    simulate_func = partial(
        simulate_iteration,
        pedigree=pedigree,
        individuals=individuals,
...
def calculate_proposal_distribution(pedigree: Pedigree, marker_set: MarkerSet, root_name: str,
    ...
    proposal[1], needed, model_probs, per_ind_probs = calculate_matching_haplotypes(
        pedigree, marker_set, root_name, suspect_haplotype, sim_params_inside, average_pedigree_probability,
        reporter, is_outside=False, number_of_threads=n_threads
    )
...
def run_simulation(
    ...
    """
    Runs the full IS Monte-Carlo pipeline (average pedigree, inside proposal, outside).
    """
    ...
    save_pedigree_to_png(pedigree=pedigree, global_config=global_config,
                         results_path=simulation_parameters.results_path, pedigree_name="original_pedigree")

    # Extended pedigree (for outside probability)
    ...
    save_pedigree_to_png(pedigree=extended_pedigree, global_config=global_config,
                         results_path=simulation_parameters.results_path, pedigree_name="extended_pedigree")

    # Extract suspect haplotype
    suspect_haplotype = deepcopy(pedigree.get_individual_by_name(suspect_name).haplotype)

    # Reduce & re-root original pedigree (inside)
    root_name = pedigree.remove_irrelevant_individuals(inside=True)

    relevant_pedigree_bytes_data = pedigree.write_to_tgf()
    with open(f"{simulation_parameters.results_path}/relevant_original_pedigree_{datetime.now().strftime('%Y%m%d%H%M%S')}.tgf", "wb") as f:
    ...
    with open(f"{simulation_parameters.results_path}/rerooted_original_pedigree_{datetime.now().strftime('%Y%m%d%H%M%S')}.tgf", "wb") as f:
        f.write(rerooted_pedigree_bytes_data)

    # Reduce & re-root extended pedigree (outside)
    extended_root_name = extended_pedigree.remove_irrelevant_individuals(inside=False, last_child_name=last_child_name)

    extended_relevant_pedigree_bytes_data = extended_pedigree.write_to_tgf()
    with open(f"{simulation_parameters.results_path}/relevant_extended_pedigree_{datetime.now().strftime('%Y%m%d%H%M%S')}.tgf", "wb") as f:
    ...
    with open(f"{simulation_parameters.results_path}/rerooted_extended_pedigree_{datetime.now().strftime('%Y%m%d%H%M%S')}.tgf", "wb") as f:
        f.write(extended_rerooted_pedigree_bytes_data)

    # Picking probabilities for inside
    if len(pedigree.individuals) > 0:
        pedigree.calculate_picking_probabilities(trace_mode)

    # Save processed pedigree for reproducibility
    timestamp = datetime.now().strftime('%Y%m%d%H%M%S')
    output_file = f"{simulation_parameters.results_path}/pedigree_{timestamp}.pkl"
    with open(output_file, "wb") as f:
        pickle.dump(pedigree, f)

    # In extended pedigree, only the last added child can be picked
    ...
    extended_pedigree.picking_probabilities = {ind.id: ind.picking_probability for ind in extended_pedigree.individuals}

    # Step 1: average pedigree probability (inside)
    max_threads = multiprocessing.cpu_count()
    number_of_threads = min(simulation_parameters.number_of_threads, max_threads)
    ...
    while True:
        try:
            ...
            # Step 2: inside proposal distribution
    ...
    # Step 3: outside match probability
